...
import qualified Data.Vector.Unboxed as V (replicate, length)
...
program c f p = programM c (const f) p
...
puti i = indexed (PUTI i) i $
    \m -> \case x:s -> setStack s <=< setMemory (m // [(i,x)])
                _ -> err "put expected an argument"

geti i = indexed (GETI i) i $ \m -> \s -> setStack ((m ! i) : s)

put = programM PUT $ 
    \m -> \case i:x:s -> setStack s <=< setMemory (m // [(i,x)])
                _ -> err "put expected an argument"
